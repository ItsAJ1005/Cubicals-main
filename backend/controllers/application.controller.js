...
class ApplicationController {
  async applyJob(req, res, next) {
    try {
      const userId = req.id; // Applicant ID
      ...
      const job = await Job.findById(jobId).populate("recruiter", "email fullname").populate("company", "name");
      if (!job) {
      ...
      if (job.recruiter?.email) {
        await sendMail({
          ...
          text: `A new applicant, ${applicant.fullname}, has applied for your job: ${job.title} at ${job.company?.name}.`,
          html: `<p>A new applicant, <strong>${applicant.fullname}</strong>, has applied for your job: <strong>${job.title}</strong> at <strong>${job.company?.name}</strong>.</p>`,
        });
      }
      ...
      if (applicant.email) {
        await sendMail({
          ...
          text: `You have successfully applied for the job: ${job.title} at ${job.company?.name}.`,
          html: `<p>You have successfully applied for the job: <strong>${job.title}</strong> at <strong>${job.company?.name}</strong>.</p>`,
        });
      }
